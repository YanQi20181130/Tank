...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    void Start() {
        ...
        EventTriggerListener.Get(nextButton).onEnter = NextScreen;
        EventTriggerListener.Get(prevButton).onEnter = PreviousScreen;
        //prevButton.GetComponent<Button>().interactable = prevButton.GetComponent<Image>().raycastTarget = false;
        //nextButton.GetComponent<Button>().interactable = nextButton.GetComponent<Image>().raycastTarget = true;
    ...
    private void LerpToPage(int aPageIndex) {
        ...
        //判断是否切换到新的页面
        if(aPageIndex==previousIndex)
        {
            //Debug.Log("same page, LerpToPage=" + aPageIndex);
        }
        else
        {
            AppDelegate.Instance.OnChangeBGEvent(aPageIndex);
            //Debug.Log("LerpToPage=" + aPageIndex);
            previousIndex = aPageIndex;
        }
        ...
        //{
        //    prevButton.GetComponent<Button>().interactable = prevButton.GetComponent<Image>().raycastTarget = true;
        //    nextButton.GetComponent<Button>().interactable = nextButton.GetComponent<Image>().raycastTarget = false;
        //}
        //else
        //{
        //    prevButton.GetComponent<Button>().interactable = prevButton.GetComponent<Image>().raycastTarget = true;
    ...
    //------------------------------------------------------------------------
    private void NextScreen(GameObject go) {
        //AudioManager.Instance.ChangeEffectSound(EffectSound.button);
        LerpToPage(_currentPage + 1);
        
    }

    //------------------------------------------------------------------------
    private void PreviousScreen(GameObject go) {
        //AudioManager.Instance.ChangeEffectSound(EffectSound.button);
        LerpToPage(_currentPage - 1);
