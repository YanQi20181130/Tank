...
public class TexturePainter : MonoBehaviour {
    ...
    public GameObject brushObj_BrushEntity;
    public GameObject brushObj_DecalEntity;
    ...
    private void Awake()
    {
        ...
        AppDelegate.hitPaintMeshDeleEvent += UpdateTexture;
    }
    public void UpdateTexture (MeshCollider meshCollider,Vector3 point) {
        brushColor = Color.yellow;// ColorSelector.GetColor ();	//Updates our painted color with the selected color

		DoAction(meshCollider,point);
		
		UpdateBrushCursor(meshCollider);
	}

	//The main action, instantiates a brush or decal entity at the clicked position on the UV map
	 void DoAction(MeshCollider meshCollider, Vector3 point)
    {	
		if (saving)
			return;
		Vector3 uvWorldPosition=Vector3.zero;		
		if(HitTestUVPosition(meshCollider,ref uvWorldPosition)){
			GameObject brushObj;
			if(mode==Painter_BrushMode.PAINT){

                brushObj=Instantiate(brushObj_BrushEntity); //Paint a brush
                                                //brushObj=(GameObject)Instantiate(Resources.Load("TexturePainter-Instances/BrushEntity")); //Paint a brush
                brushObj.GetComponent<SpriteRenderer>().color=brushColor; //Set the brush color
			}
			else{
				
                brushObj = Instantiate(brushObj_DecalEntity); //Paint a decal
            ...
            brushObj.transform.SetParent(brushContainer.transform,false);
	...
	void UpdateBrushCursor(MeshCollider meshCollider)
    {
		Vector3 uvWorldPosition=Vector3.zero;
		if (HitTestUVPosition (meshCollider,ref uvWorldPosition) && !saving) {
	...
	bool HitTestUVPosition(MeshCollider meshCollider,ref Vector3 uvWorldPosition){
		RaycastHit hit;
